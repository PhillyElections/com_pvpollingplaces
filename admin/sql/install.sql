...
SELECT DISTINCT `pin_address`, `location`, 1, @tnow FROM `#__pollingplaces`;
...
SELECT `p`.`id`, `d`.`id`, 1, @tnow FROM `#__pollingplaces` as `p`, `#__divisions` as `d` where `d`.`division_id`=`p`.`division_id`;
